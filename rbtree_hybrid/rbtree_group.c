...
unsigned long long int cnt_node_array = 0;
unsigned long long int cnt_key_list = 0;
int GROUP_SIZE = 0; 
int WSS = 0;
...
unsigned long long int max_list_size = 0; //1010(10) >> 101 (5) >> 10 (2) >> 1(1)

void set_max_list_size(int gs) {
	int tmp = gs;
	while (tmp > 1) {
		tmp = tmp >> 1;
		max_list_size++;
...
void init_rb_key_array(simul_rb_key_array *key_node, long long int key) {
	key_node->rb_key = key;
...
void init_rb_key_list(simul_rb_key_list *key_node, long long int key) {
        key_node->rb_key = key;
...
void init_rb_node(simul_rb_root *T, simul_rb_node *node, simul_rb_key_list *key_node) {
        set_rb_red(node);
        node->std_key = key_node->rb_key / (GROUP_SIZE);
        init_list_head(&(node->rb_keys_list));
        list_add(&(key_node->list), &(node->rb_keys_list));
	node->rb_keys_array = NULL;
        node->cnt_key = 1;
...
void init_rb_root(simul_rb_root *root) {
	...
	init_list_head(&(root->nil_node->rb_keys_list));
	root->nil_node->rb_keys_array = NULL;
...
*/
	...
	init_list_head(&(root->root_node->rb_keys_list));
	root->root_node->rb_keys_array = NULL;
...
int simul_rb_insert(simul_rb_root *T, long long int inkey) {
	...
	simul_rb_key_array *new_key_array = NULL;
	simul_rb_key_list *new_key_list = NULL;
        simul_rb_key_list *pre_cur_key_list = NULL, *cur_key_list = NULL;
...
//		return SIMUL_RB_EXIST;
	...
	if (is_in_group == 1) {	
		// check duplicate
		if (comp_node->is_array == 1) {	// array
			if (comp_node->rb_keys_array[inkey % (GROUP_SIZE)].rb_key != 0)
				return SIMUL_RB_EXIST;
		} else {	// list
		        pre_cur_key_list = &(comp_node->rb_keys_list);
		        cur_key_list = container_of(comp_node->rb_keys_list.next, simul_rb_key_list, list);
		        cur_list = comp_node->rb_keys_list.next;
		
		        while (cur_list != &comp_node->rb_keys_list) {
		                if (cur_key_list->rb_key == inkey) {
					return SIMUL_RB_EXIST;
		                }
		                pre_cur_key_list = cur_key_list;
		                cur_key_list = container_of(cur_key_list->list.next, simul_rb_key_list, list);
		                cur_list = cur_list->next;
		        }
		}

		if (comp_node->is_array == 1) {	// array
			comp_node->rb_keys_array[inkey % (GROUP_SIZE)].rb_key = inkey;
			//	comp_node->rb_keys[new_key->rb_key % GROUP_SIZE].area = new_key->area;
			comp_node->cnt_key++;
			return SIMUL_RB_IN_GROUP;
		} else if (comp_node->cnt_key == max_list_size) {	// list -> array
			comp_node->rb_keys_array = (struct simul_rb_key_array *) malloc (sizeof(struct simul_rb_key_array) * (GROUP_SIZE));
			memset(comp_node->rb_keys_array, 0, sizeof(struct simul_rb_key_array) * (GROUP_SIZE));
			...
			// copy list to array
		        ...
		        while (cur_list != &comp_node->rb_keys_list) {
				comp_node->rb_keys_array[cur_key_list->rb_key % (GROUP_SIZE)].rb_key = cur_key_list->rb_key;
		                pre_cur_key_list = cur_key_list;
		                cur_key_list = container_of(cur_key_list->list.next, simul_rb_key_list, list);
		                cur_list = cur_list->next;
				list_del(&(pre_cur_key_list->list));
				free(pre_cur_key_list);
		        	cnt_key_list--;
		        }
			...
			//	comp_node->rb_keys[new_key->rb_key % GROUP_SIZE].area = new_key->area;
			comp_node->cnt_key++;
			comp_node->is_array = 1;
			cnt_node_array++;

			return SIMUL_RB_IN_GROUP;		
		} else {	// list
		        new_key_list = (simul_rb_key_list *)malloc(sizeof(simul_rb_key_list));
		        init_rb_key_list(new_key_list, inkey);
		        cnt_key_list++;

	                list_add(&(new_key_list->list), &(comp_node->rb_keys_list));      // connect list : second -> first -> second->next
	        //      comp_node->rb_keys[new_key->rb_key % GROUP_SIZE].area = new_key->area;
	                comp_node->cnt_key++;
	                return SIMUL_RB_IN_GROUP;
		}
	...
	// init node
	init_rb_node(T, new_node, new_key_list);
	cnt_node++;
...
int simul_rb_delete(simul_rb_root *T, long long int delkey) {
	...
	if (del_node->is_array == 1) {	// array
		if (del_node->rb_keys_array[delkey % (GROUP_SIZE)].rb_key == 0)
			return SIMUL_RB_NOT_EXIST;

		memset(&(del_node->rb_keys_array[delkey % (GROUP_SIZE)]), 0, sizeof(simul_rb_key_array));
		del_node->cnt_key--;
	} else {	// list
	        pre_cur_key_list = &(del_node->rb_keys_list);
	        cur_key_list = container_of(del_node->rb_keys_list.next, simul_rb_key_list, list);
	        cur_list = del_node->rb_keys_list.next;
	
	        while (cur_list != &del_node->rb_keys_list) {
	                if (cur_key_list->rb_key == delkey) {
	                        is_in_group = 1;
	                        break;
	                }
	                pre_cur_key_list = cur_key_list;
...
long long int simul_rb_search_key(simul_rb_root *T, long long int find_key) {
	simul_rb_node *find_node = simul_rb_search(T, find_key);
	simul_rb_key_list *pre_cur_key_list = NULL, *cur_key_list = NULL;
	struct simul_list_head *cur_list = NULL;
	...
	if (find_node->is_array == 1)	// array
		retval = find_node->rb_keys_array[find_key % (GROUP_SIZE)].rb_key;
	else {
	        pre_cur_key_list = &(find_node->rb_keys_list);
	        cur_key_list = container_of(find_node->rb_keys_list.next, simul_rb_key_list, list);
	        cur_list = find_node->rb_keys_list.next;

	        while (cur_list != &find_node->rb_keys_list) {
	                if (cur_key_list->rb_key == find_key) {
				retval = cur_key_list->rb_key;
	                        break;
	                }
	                pre_cur_key_list = cur_key_list;
	                cur_key_list = container_of(cur_key_list->list.next, simul_rb_key_list, list);
...
void simul_rb_delete_all(simul_rb_root *T, simul_rb_node *node) {
        simul_rb_key_list *cur_key = NULL, *pre_key = NULL;
        struct simul_list_head *cur_list = NULL;
...
#endif
	if (node->is_array == 1) {	// array
		free(node->rb_keys_array);
	} else {	// list
	        cur_key = container_of(node->rb_keys_list.next, simul_rb_key_list, list);
	        pre_key = &(node->rb_keys_list);
	        cur_list = node->rb_keys_list.next;

	        while (cur_list != &node->rb_keys_list) {
	                pre_key = cur_key;
	                cur_key = container_of(cur_key->list.next, simul_rb_key_list, list);
	                cur_list = cur_list->next;
	                free(pre_key);
	        }
	}
...
//	simul_rb_key *ser_key = NULL;
	FILE *fp = NULL;
	long long int in_key = -1, i = 0, *rvalue, ser_key = -1;
...
//	init_swap_node();
		...
		gettimeofday(&serstart, NULL);
			...
			if (argc < 5)
				...
				key = rvalue[i];
	                        ...
	                        key = rand() % ((WSS)*atoi(argv[4]));
                        ser_key = simul_rb_search_key(root, (long long int)key);
                }
	...
	//		simul_rb_swapping(root);
		...
		gettimeofday(&serstart, NULL);
                for(i = 0; i<WSS;i++){
                        ser_key = simul_rb_search_key(root, (long long int)i);
                }
		gettimeofday(&serend, NULL);
	...
	sizeofmem = cnt_node_array * (sizeof(unsigned int) + sizeof(long long int) + (sizeof(struct simul_rb_node *)*3) + sizeof(unsigned int) + (sizeof(long long int)*(GROUP_SIZE))) + ((cnt_node - cnt_node_array) * (sizeof(unsigned int) + sizeof(long long int) + (sizeof(struct simul_rb_node *)*3) + sizeof(unsigned int))) + (cnt_key_list * (sizeof(struct simul_list_head)));
	...
	printf(" number of node : %llu, array : %llu\n", cnt_node, cnt_node_array);
#if DEBUG_WRITE_COUNT
